...
export default function ActiveBookingScreen() {
  ...
  const parking = state.parking || mockParkings[0]
  const hours = state.hours || 2
  const selectedVehicle = state.selectedVehicle || user.car || user.vehicles?.[0]
  ...
  useEffect(() => {
    const iv = setInterval(() => setElapsed(e => {
      if (e >= maxSeconds) { clearInterval(iv); return e }
      return e + 1
    }), 1000)
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#0a0a0a] px-5 pb-8">
      <div className="flex items-center gap-3 pt-12 pb-6">
        ...
        <div>
          <h1 className="text-white text-xl font-bold">{t('activeParking')}</h1>
          <p className="text-[#8B8B8B] text-sm">{t('sessionInProgress')}</p>
        </div>
      </div>

      {/* Timer */}
      <div className="bg-[#1a1a1a] rounded-3xl p-8 mb-4 text-center">
        <p className="text-[#8B8B8B] text-xs mb-1">{t('timeRemaining')}</p>
        ...
        <div className="flex justify-between mt-2">
          <span className="text-[#8B8B8B] text-xs">{t('started')}</span>
          <span className="text-[#8B8B8B] text-xs">{hours}h {t('max')}</span>
        </div>
      </div>

      {/* Live cost */}
      <div className="bg-[#FF6B00]/10 border border-[#FF6B00]/30 rounded-2xl p-4 mb-4 text-center">
        <p className="text-[#8B8B8B] text-xs">{t('currentCost')}</p>
      ...
      <div className="bg-[#FF6B00]/10 border border-[#FF6B00]/30 rounded-2xl p-4 mb-6 flex items-center gap-3">
        ...
        <div>
          <p className="text-[#8B8B8B] text-xs">{t('yourSpot')}</p>
          <p className="text-white font-bold text-lg">White Zone · A-013</p>
        </div>
      </div>

      {/* End Parking button only */}
      <div className="mt-auto">
        <button
          onClick={handleEndParking}
          className="w-full py-4 rounded-2xl bg-[#FF6B00] text-white font-bold"
        >
          {t('endParking')}
        </button>
      </div>
